...
function MapOverview() {
  ...
  const stats = [
    { label: t('Total Vehicles'), value: 12, icon: Truck, color: 'blue' },
    { label: t('Active Trips'), value: 8, icon: Navigation, color: 'green' },
    { label: t('Alerts'), value: 2, icon: AlertCircle, color: 'red' },
    { label: t('Drivers Online'), value: 11, icon: Users, color: 'purple' },
  ];
    ...
    <div className="space-y-6 pb-20 lg:pb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-sm p-4">
            <div className="w-full h-96 bg-gray-100 rounded-lg flex items-center justify-center">
              <div className="text-center">
                <MapPin className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                <p className="text-gray-600 font-medium">{t('Interactive Map')}</p>
                <p className="text-sm text-gray-500">{t('Displaying live vehicle locations')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {/* Active Vehicles */}
          <div className="bg-white rounded-lg shadow-sm p-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">{t('Active Vehicles')}</h3>
            <div className="space-y-3">
              {vehicles.map((vehicle) => (
                <div key={vehicle.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                  ...
                  <div className="text-right">
                    <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${
                      ...
                      {vehicle.status === 'In Transit' ? t('In Transit') : t('Idle')}
                    </span>
                    <p className="text-xs text-gray-500 mt-1">{vehicle.speed} km/h</p>
                  </div>
                </div>
          ...
          {/* Active Alerts */}
          <div className="bg-white rounded-lg shadow-sm p-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">{t('Active Alerts')}</h3>
            <div className="space-y-3">
              {alerts.map((alert) => (
                <div
                  key={alert.id}
                ...
                >
                  <p className="text-sm font-medium text-gray-800">{t(alert.type)}</p>
                  <p className="text-xs text-gray-600">{alert.vehicle}</p>
                </div>
              ))}
            </div>
      ...
      {/* Fleet Summary */}
      <div className="bg-white rounded-lg shadow-sm p-5">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">{t('Fleet Summary')}</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 text-center">
          <div><p className="text-2xl font-bold text-emerald-600">8</p><p className="text-xs text-gray-500">{t('In Transit')}</p></div>
          <div><p className="text-2xl font-bold text-blue-600">3</p><p className="text-xs text-gray-500">{t('Idle')}</p></div>
          <div><p className="text-2xl font-bold text-gray-600">1</p><p className="text-xs text-gray-500">Offline</p></div>
          <div><p className="text-2xl font-bold text-orange-600">234</p><p className="text-xs text-gray-500">{t('Total km')}</p></div>
          <div><p className="text-2xl font-bold text-purple-600">45.2</p><p className="text-xs text-gray-500">{t('Avg Speed')}</p></div>
          <div><p className="text-2xl font-bold text-red-600">2</p><p className="text-xs text-gray-500">{t('Alerts')}</p></div>
        </div>
      </div>
    </div>
  );
